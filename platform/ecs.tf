...
resource "aws_ecs_cluster" "main_fargate_cluster" {
  name = "Main-Fargate-Cluster"
}

resource "aws_alb" "ecx_cluster_alb" {
...
resource "aws_alb_listener" "ecs_alb_https_listener" {
  load_balancer_arn = aws_alb.ecx_cluster_alb.arn
  port              = 443
  protocol          = "HTTPS"
...
resource "aws_route53_record" "ecs_load_balancer_record" {
  ...
  alias {
    ...
    name                   = aws_alb.ecx_cluster_alb.dns_name
    zone_id                = aws_alb.ecx_cluster_alb.zone_id
  }
}
